...
const UserCard: React.FC<UserCardProps> = ({ session, onSessionCancelled }) => {
  ...
  const handleConfirmCancel = async () => {
    ...
    try {
      await cancelSession(sessionkey);
      onSessionCancelled(); // Trigger refresh in the parent component
    } catch (error) {
      console.error('Error cancelling session:', error);
      alert(`Falha ao cancelar a sessão de ${username}. Por favor, tente novamente.`);
    } finally {
  ...
  return (
    <>
      ...
      <ConfirmModal
        ...
        message={`Tem certeza que deseja remover a sessão do usuário ${username}? Esta ação não pode ser desfeita.`}
